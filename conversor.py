...
class ConversorMapasFrame(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ctk.CTkLabel(
            self, text="Conversor de Execução Orçamentária", font=("Segoe UI", 28, "bold"), text_color=HEADER_COLOR
        ).pack(pady=(30, 20))
        ...
        ctk.CTkLabel(
            ...
            font=("Segoe UI", 14), wraplength=600, justify="center", text_color=TEXT_COLOR_GRAY
        ...
        self.btn_anexar_inex = ctk.CTkButton(
            btn_frame, text="📎 Anexar INEX", command=self._anexar_inex,
            fg_color=SIDEBAR_BTN_FG, hover_color="#1D67B5", text_color=HEADER_COLOR,
            font=("Segoe UI", 18, "bold"), width=180, height=60, corner_radius=12
        )
        self.btn_anexar_inex.pack(side="left", padx=12, pady=4)

        self.btn_converter = ctk.CTkButton(
            btn_frame, text="📤 Gerar extrato", command=self._converter,
        ...
        self.status = ctk.CTkLabel(
            self, text="📂 Selecione um mapa para gerar o relatório.", font=("Calibri", 14, "bold"), text_color="#60A5FA"
        )
        self.status.pack(pady=20)

    def atualizar_arquivo_mapa(self, path: str):
        self.arquivo_mapa = path
        self.status.configure(text=f"📄 Arquivo selecionado: {os.path.basename(path)}", text_color="#60A5FA")
    ...
    def _anexar_inex(self):
        ...
        self.status.configure(text=f"📄 INEX anexado: {os.path.basename(path)}", text_color="#60A5FA")
    ...
    def _converter(self):
        ...
        self.progress.pack(fill="x", padx=30, pady=10)
        self.progress.start()
        self.status.configure(text="⏳ Processando...", text_color="#60A5FA")
        self.update()
        ...
        try:
            ...
            caminho_completo = os.path.realpath(os.path.join(self.pasta_destino, nome_arquivo))
            ...
            self.status.configure(text=f"✅ Arquivo salvo: {nome_arquivo}", text_color="#0E9E66")
    ...
    def _processar(self) -> pd.DataFrame:
        ...
        if faltando: raise ValueError(f"Colunas faltando no mapa: {', '.join(sorted(faltando))}")

        mapa_df["CNPJ"] = mapa_df["CNPJ"].replace(["", " ", "0", "0.0", "nan", "None", None], pd.NA)
        mapa_df["Identificador"] = mapa_df["CNPJ"].fillna(mapa_df["CPF"])

        resultado = (
            mapa_df.groupby(["Identificador", "Plano Interno"], dropna=False).agg(
                Nome = ("Nome", "first"),
                Fatura = ("Fatura", lambda x: ", ".join(str(v).split(".")[0] if str(v).replace(".", "").isdigit() else str(v) for v in x.unique())),
                Valor = ("Valor", "sum"),
            ).reset_index().rename(columns={"Identificador": "CNPJ/CPF"})
        )
    ...
    def _mesclar_inex(self, resultado: pd.DataFrame) -> pd.DataFrame:
        ...
        if "ITEM" not in inex_df.columns: inex_df["ITEM"] = ""
        if "INEX" not in inex_df.columns: inex_df["INEX"] = ""
